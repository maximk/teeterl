...
pattern({named_tuple,Line,Tag,Ps}, St0) ->
    {TPs,TPsvs,St1} = pattern_list(Ps, St0),
    {{named_tuple,Line,Tag,TPs},TPsvs,St1};
pattern({named_tuple_field,Line,Field,P}, St0) ->
	{TP,St1} = pattern(P, St0),
	{{named_tuple_field,Line,Field,TP}, St1};
...
expr({record_field,_,_,_}=M, St) ->
    {expand_package(M, St),St};  % must be a package name
expr({named_tuple,Line,Tag,Es}, St0) ->
    {Es1,St1} = expr_list(Es, St0),
    {{named_tuple,Line,Tag,Es1},St1};
expr({named_tuple_field,Line,Field,E}, St0) ->
	{TE,St1} = expr(E, St0),
	{{named_tuple_field,Line,Field,TE}, St1};
