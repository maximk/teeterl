...
%%MK 5.9.2009
%% Makefile interface
-export([files_outdir/1,files_outdir_binary/1,files_outdir_dopjop/1]).
%%MK
...
%%MK 5.9.2009
files_outdir(FilesOutdir) ->
	[Outdir|Files] = reverse(FilesOutdir),
	foreach(fun(File) ->
		file(File, [{outdir,Outdir}|?DEFAULT_OPTIONS])
	end, Files).
files_outdir_binary(FilesOutdir) ->
	[Outdir|Files] = reverse(FilesOutdir),
	foreach(fun(File) ->
		file(File, [{outdir,Outdir},binary|?DEFAULT_OPTIONS])
	end, Files).
files_outdir_dopjop(FilesOutdir) ->
	[Outdir|Files] = reverse(FilesOutdir),
	foreach(fun(File) ->
		file(File, [{outdir,Outdir},dopjop|?DEFAULT_OPTIONS])
	end, Files).
%%MK
...
internal({file,File}, Opts) ->
	...
	false ->
		...
		true ->
		    internal_comp(Ps, File, ".x", Compile);
		false ->
...
kernel_passes() ->
    ...
    [{unless,no_constant_pool,?pass(core_dsetel_module)}, %Not safe without constant pool.
     ...
     %% {iff,seekit,{pass,tt_seek}},	%% MK 18.9.2009
     {pass,tt_codegen},		%%MK 7.9.2009
     %% {iff,dcg,{listing,"codegen"}}		%% MK 15.9.2009
     {iff,dcg,?pass(codegen_listing)}			%% MK 15.9.2009
     | asm_passes()].
...
%%MK 15.9.2009
	[{unless,no_jumpl,?pass(asm_jump_next)},
	 {iff,djumpl,?pass(asm_listing)},
	 {unless,no_ulab,?pass(asm_unused_labels)},
	 {iff,dulab,?pass(asm_listing)},
	 {unless,no_labj,?pass(asm_labeled_jumps)},
	 {iff,dlabj,?pass(asm_listing)},
	 {unless,no_opjop,?pass(asm_op_jump_op)},
	 {iff,dopjop,?pass(asm_listing)},
	 ?pass(asm_preload)
	| binary_passes()].
%%MK
...
%%MK
	[{iff,binary,?pass(write_binary_preloaded)},
	 {unless,binary,?pass(write_text_preloaded)}].

%%MK 15.9.2009
codegen_listing(St) ->
	io:format("Codegen:~n~p~n", [St#compile.code]),
	{ok,St}.

asm_op_jump_op(St) ->
	case tt_asm:opt_op_jump_op(St#compile.code) of
	{ok,Code} -> {ok,St#compile{code=Code}};
	{error,Es} ->
		Errors = [{St#compile.ifile,Es}],
		{error,St#compile{errors=Errors}}
	end.

asm_jump_next(St) ->
	case tt_asm:opt_jump_next(St#compile.code) of
	{ok,Code} -> {ok,St#compile{code=Code}};
	{error,Es} ->
		Errors = [{St#compile.ifile,Es}],
		{error,St#compile{errors=Errors}}
...
asm_unused_labels(St) ->
	case tt_asm:opt_unused_labels(St#compile.code) of
	{ok,Code} -> {ok,St#compile{code=Code}};
	{error,Es} ->
		Errors = [{St#compile.ifile,Es}],
		{error,St#compile{errors=Errors}}
...
asm_labeled_jumps(St) ->
	case tt_asm:opt_labeled_jumps(St#compile.code) of
	{ok,Code} -> {ok,St#compile{code=Code}};
	{error,Es} ->
		Errors = [{St#compile.ifile,Es}],
		{error,St#compile{errors=Errors}}
...
asm_preload(St) ->
	{ok,Teevm} = tt_asm:preload(St#compile.code),
	{ok,St#compile{code=Teevm}}.

asm_listing(St) ->
	tt_asm:listing_with_offsets(St#compile.code),
	{ok,St}.
	
%% MK 28.09.2009
write_binary_preloaded(St) ->
	tt_asm:write_binary_preload(St#compile.ofile, St#compile.code),
	{ok,St}.
	
write_text_preloaded(St) ->
	tt_asm:write_text_preload(St#compile.ofile, St#compile.code),
	{ok,St}.
%%MK
...
objfile(Base, St) ->
	case member(binary, St#compile.options) of
	true ->
		outfile(Base, "x", St#compile.options);
	false ->
		outfile(Base, "cx", St#compile.options)
	end.
