...
%% possible is moved to the receiving side using lists_reverse/2 when data
...
split(N, {R,F}=Q) when is_integer(N), N > 0, is_list(R), is_list(F) ->
    Lf = erlang:length(F),
    if  N < Lf ->
	    {F1,F2} = lists_split(N, F),
	    {f2r(F1),{R,F2}};
        N > Lf ->
	    case catch lists_split(erlang:length(R)-(N-Lf), R) of
		{'EXIT',_} ->
...
f2r([H1,H2]) ->
    {[H2],[H1]};
f2r([H1,H2|T]) ->
    {lists_reverse(T, []),[H1,H2]}.

%% copied from 'lists'

lists_reverse(L) -> lists_reverse(L, []).
lists_reverse([H|T], L) -> lists_reverse(T, [H|L]);
lists_reverse([], L) -> L.

lists_split(N, List) when is_integer(N), N >= 0, is_list(List) ->
    case lists_split(N, List, []) of
	{_, _} = Result -> Result;
