...
%%%%
io_format(Fmt) ->
	comet:post_message("/test/out", Fmt).
io_format(Fmt, Args) ->
	comet:post_message("/test/out", {Fmt,Args}).
tc(M, F, A) ->
    Before = erlang:now(),
...
get_cpu_speed({win32, _},_DataDir) ->
	...
	case catch win32reg:open([read]) of
		...
		Handle;
	    _ ->
		io_format("Error.~nCannot determine CPU clock"
			  "frequency.~n"
	...
	ok ->
	    ok;
	_ ->
	    io_format("Error.~nRegistry seems to be damaged or"
		      "unavailable.~n"
		      "Please set the environment variable"
	...
	_ ->
	    io_format("Error.~nRegistry seems to be damaged or "
		      "unavailable.~n"),
	    io_format("Please set the environment variable"
		      "\"CPU_SPEED\"~n"),
	    win32reg:close(RegH),
	    exit(self(), {error, no_cpu_speed})
    end;
...
pp(Mhz,Total,Stones,Ms) ->
    io_format("EStone test completed~n",[]),
    io_format("**** CPU speed ~s MHz ****~n",[Mhz]),
    io_format("**** Total time ~w seconds ****~n", [Total / 1000000]),
    io_format("**** ESTONES = ~w ****~n~n", [Stones]),
    io_format("~-31s      ~-12s  ~-10s   %    ~-10s ~n~n",
...
pp2([]) -> ok;
pp2([R|Tail]) ->
    io_format("~-35s  ~-12w    ~-10w   ~-2w    ~-10w ~n",
	      [ks(title,R), 
...
pat_loop3(_I, [1, 0 |_T]) ->
    ok;
pat_loop3(_I, [X, _Y |_Tail]) when is_binary(X) andalso size(X) == 1 ->
    ok;
...
binary_h_2(I, P, B) ->
    ...
    split_loop(B, {abc,1,2222,1.2345,"ancnd"}, 100),
    binary_h_2(I-1, P, B).
...
recv(N, [Timeout|Tail], L) ->
    receive
        hi_dude ->
            recv(N-1, Tail, L)
    after Timeout ->
            io_format("XXXXX this wasn't supposed to happen???~n", []),
            ok
    end.
